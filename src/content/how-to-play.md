...
3. **Explore**: flip location cards one at a time, in ID order (A, then B, then C), and resolve what's on the back before moving to the next. An optional card, shown with its ID in parentheses, can be skipped by moving on to the next card; it turns back face down, and you can come back to it any time before the Scene ends. Combat happens here too.
